...
function groupByCity(data) {
    ...
    // Iterate over each route to produce an object where each key is a city, and each value holds information about that city.
    let result = data.reduce((result, d) => {
    ...
    // Convert the result object into a list for easier visualization.
    result = Object.keys(result).map(key => result[key]);
    result = result.sort((a, b) => b.Count - a.Count); // Sort the data in descending order of count.
    return result;
}
